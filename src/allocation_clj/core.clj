(ns allocation_clj.core
  ...
  (import [org.jclouds.abiquo.domain.infrastructure Datacenter Rack Machine Datastore]
          ...
          [org.jclouds.abiquo.reference AbiquoEdition]
          [org.jclouds.abiquo.predicates.infrastructure DatacenterPredicates HypervisorPredicates]
          [org.jclouds.abiquo.domain.cloud VirtualDatacenter VirtualAppliance]
          [org.jclouds.abiquo.domain.network PrivateNetwork]
...
(defn- create
  []
  (let [dcbuilder (Datacenter/builder context)]
         (. dcbuilder (name "Morgul Mines"))
         (. dcbuilder (location "morgul"))
         (. dcbuilder (remoteServices "10.60.1.224" (AbiquoEdition/ENTERPRISE)))
         (.. dcbuilder (build) (save))))
(defn create-machine
  [datacenter rack]
                     ...
                     (doseq [i (range 0 1)]
                                  (. rbuilder (name (random-str 17)))
                                   (let [r (. rbuilder (build))]
                                      (. r (save)))))))
  ([dc]
    (let [builder (Rack/builder context dc)]
           (. builder (name (random-str 17)))
           (.. builder (build) (save))))
  ([dc name]
    (let [builder (Rack/builder context dc)]
           (. builder (name name))
...
(def ent (.. context (getAdministrationService) (findEnterprise (EnterprisePredicates/name (into-array ["Abiquo"])))))


(defn create-virtualdatacenter
  ([name]
  (let [
        dc (first dcs)
        hypervisor (. dc (findHypervisor 
                           (HypervisorPredicates/type (into-array [(HypervisorType/VMX_04)]))))
        networkb (PrivateNetwork/builder context)
        builder (VirtualDatacenter/builder context dc ent)]
          (. networkb (name "network"))
          (. networkb (gateway "192.168.1.1"))
          (. networkb (address "192.168.1.0"))
          (. networkb (mask 24))
          (. builder (name name))
          (. builder ( hypervisorType hypervisor))
          (. builder (network (. networkb (build))))
          (.. builder (build) (save))))
  ([dc name]
    (let [hypervisor (. dc (findHypervisor 
                           (HypervisorPredicates/type (into-array [(HypervisorType/VMX_04)]))))
        networkb (PrivateNetwork/builder context)
        builder (VirtualDatacenter/builder context dc ent)]
          (. networkb (name "network"))
...
(defn create-virtualappliance
  [vdc name]
  (let [builder (VirtualAppliance/builder context vdc)]
        (. builder (name name))
        (.. builder (build) (save))))
...
(defn -main
  ...
  (if (< (. dcs (size)) 2)
    (do 
      ;;(create)
      ;;(create-racks)
      ;;(create-racks (create) (random-str 17))
      ;;(create-machine (first dcs) (first (. (first dcs) (listRacks))))
      (create-machine (create-racks (create)))
      ;;(create-virtualdatacenter "esxi")
      (create-virtualappliance (create-virtualdatacenter "esxi") "esxi"))
   ))  
